...
public class RunOnSystemStartup {
   ...
   * 
   * @param className
   * Name of class within the current JAR file to run on system startup.
   * @param windowTitle
  ...
  public static void install(String className, String windowTitle) throws Exception {
    ...
    if (osName.startsWith("Windows")) {
      ...
      out.println("start \"" + windowTitle + "\" \"" + javaHome + fileSeparator + "bin" + fileSeparator + "java.exe\" -cp " + getJarFile() + " "
          + className);
    } else if (osName.startsWith("Mac OS")) {
